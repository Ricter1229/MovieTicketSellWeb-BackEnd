...
public class StoreReleaseMovieController {
	...
	public ApiResponse<Object> updateStoreMovie(@RequestBody StoreReleaseMovieRequestDto request) {
		...
		List<StoreReleaseMovieBean> reqponse = storeReleaseMovieService.insert(request.getStoreReleaseMovie().get(0).getStoreId(), request.getStoreReleaseMovie().get(0).getMovieVersionIds());
	...
	public ApiResponse<Object> getAllWithStoreIdAndVersionId(@RequestBody StoreReleaseMovieRequestDto request) {
		return ApiResponse.success(storeReleaseMovieService.findByStoreIdAndVersionId(request.getStoreId(), request.getVersionId()));
	}
